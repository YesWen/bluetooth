...
import { arrayBufferToHex, handleData } from '@/common/bluetooth/helper.js'
...
const onBluetoothDeviceFound = async () => {
    uni.onBluetoothDeviceFound(({ devices }) => {
        devices.filter(({ name, localName }) => name || localName).forEach((device) => {
...
const repetitionServices = async (deviceId) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        let servicesList = services.filter((item) => { return !/^000018/.test(item.uuid) });
