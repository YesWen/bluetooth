import { plus } from "@/utils/numberPrecision";
...
//计算校验值
const proofTestValue = (arr) => {
    return arr.reduce((prev, current) => plus(prev, parseInt(current, 16)), 0);
    let length = hex.length / 2;
    let start = 0;
    let end = 2;
    let res = 0;
    for (let i = 0; i < length; i++) {
        let curData = parseInt(hex.substring(start, end), 16);
        res += curData;
        start += 2;
        end += 2;
    }
    let result = res.toString(16).length == 2 ? res.toString(16) : res.toString(16).substring(1, res.length);
    console.log(result);
};

// '5A 5A 0F 01 1D 0203181A6A102000202A50D0A'
